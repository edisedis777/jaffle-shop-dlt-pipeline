# Complete Jaffle Shop dlt Pipeline with Performance Optimizations
import dlt
...
def jaffle_shop_source_optimized():
    ...
    def get_customers() -> Iterator[Dict[str, Any]]:
        ...
        page = 1
        while True:
            response = client.get("/customers", params={"page": page, "limit": 1000}) 
            data = response.json()
            if isinstance(data, list):
                items = data
            else:
    ...
    def get_orders() -> Iterator[Dict[str, Any]]:
        ...
        page = 1
        while True:
            response = client.get("/orders", params={"page": page, "limit": 1000}) 
            data = response.json()
            if isinstance(data, list):
                items = data
            else:
    ...
    def get_products() -> Iterator[Dict[str, Any]]:
        ...
        page = 1
        while True:
            response = client.get("/products", params={"page": page, "limit": 1000})
            data = response.json()
            if isinstance(data, list):
                items = data
            else:
...
def run_performance_comparison():
    ...
    pipeline_naive = dlt.pipeline(
        ...
        destination="duckdb",
        dataset_name="jaffle_data_naive"
    )
    ...
    pipeline_optimized = dlt.pipeline(
        ...
        destination="duckdb",
        dataset_name="jaffle_data_optimized"
    )
    start_time = time.time()
    ...
    try:
        ...
        customers_count = conn_naive.execute("SELECT COUNT(*) FROM jaffle_data_naive.customers").fetchone()[0]
        orders_count = conn_naive.execute("SELECT COUNT(*) FROM jaffle_data_naive.orders").fetchone()[0]  
        products_count = conn_naive.execute("SELECT COUNT(*) FROM jaffle_data_naive.products").fetchone()[0]
        ...
        customers_count_opt = conn_opt.execute("SELECT COUNT(*) FROM jaffle_data_optimized.customers").fetchone()[0]
        orders_count_opt = conn_opt.execute("SELECT COUNT(*) FROM jaffle_data_optimized.orders").fetchone()[0]
        products_count_opt = conn_opt.execute("SELECT COUNT(*) FROM jaffle_data_optimized.products").fetchone()[0]
